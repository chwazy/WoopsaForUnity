...
namespace Woopsa
{
    public class WoopsaClientSubscriptionChannel
    {
        ...
        private async void RequestAllUnregistration()
        {
            lock (_subscriptions)
            {
                foreach (var item in _subscriptions)
        ...
        /// <summary>
        /// Modified version : replaced threads by tasks so it
        /// works in Unity.
        /// </summary>
        private void EnsureServiceThreadStarted()
        {
            ...
            {
                _taskNotifications = new Task(executeServiceNotifications);
                _taskNotifications.Start();
            }
            if (_taskSubscriptions == null)
            {
                _taskSubscriptions = new Task(executeServiceSubscriptions);
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                if (_taskSubscriptions != null)
                {
                    _taskSubscriptions.Dispose();
                }
                if (_taskNotifications != null)
                {
                    _taskNotifications.Dispose();
                }
            }
        }
